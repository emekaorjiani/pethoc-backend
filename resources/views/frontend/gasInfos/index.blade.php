        ...
        <div class="col-md-12">
            @can('gas_info_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.gas-infos.create') }}">
                            ...
                            <tbody>
                                @foreach($gasInfos as $key => $gasInfo)
                                    <tr data-entry-id="{{ $gasInfo->id }}">
                                        ...
                                        <td>
                                            @can('gas_info_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.gas-infos.show', $gasInfo->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('gas_info_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.gas-infos.edit', $gasInfo->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('gas_info_delete')
                                                <form action="{{ route('frontend.gas-infos.destroy', $gasInfo->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('gas_info_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.gas-infos.massDestroy') }}",
