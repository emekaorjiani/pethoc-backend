        ...
        <div class="col-md-12">
            @can('pump_transaction_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.pump-transactions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $pumpTransaction->id }}">
                                        ...
                                        <td>
                                            @can('pump_transaction_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.pump-transactions.show', $pumpTransaction->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('pump_transaction_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.pump-transactions.edit', $pumpTransaction->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('pump_transaction_delete')
                                                <form action="{{ route('frontend.pump-transactions.destroy', $pumpTransaction->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pump_transaction_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.pump-transactions.massDestroy') }}",
