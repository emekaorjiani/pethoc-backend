        ...
        <div class="col-md-12">
            @can('sales_transactions_history_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.sales-transactions-histories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $salesTransactionsHistory->id }}">
                                        ...
                                        <td>
                                            @can('sales_transactions_history_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.sales-transactions-histories.show', $salesTransactionsHistory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('sales_transactions_history_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.sales-transactions-histories.edit', $salesTransactionsHistory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('sales_transactions_history_delete')
                                                <form action="{{ route('frontend.sales-transactions-histories.destroy', $salesTransactionsHistory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('sales_transactions_history_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.sales-transactions-histories.massDestroy') }}",
