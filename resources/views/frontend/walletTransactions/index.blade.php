        ...
        <div class="col-md-12">
            @can('wallet_transaction_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.wallet-transactions.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $walletTransaction->id }}">
                                        ...
                                        <td>
                                            @can('wallet_transaction_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.wallet-transactions.show', $walletTransaction->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('wallet_transaction_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.wallet-transactions.edit', $walletTransaction->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('wallet_transaction_delete')
                                                <form action="{{ route('frontend.wallet-transactions.destroy', $walletTransaction->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('wallet_transaction_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.wallet-transactions.massDestroy') }}",
