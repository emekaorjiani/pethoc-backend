        ...
        <div class="col-md-12">
            @can('distributor_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.distributors.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $distributor->id }}">
                                        ...
                                        <td>
                                            @can('distributor_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.distributors.show', $distributor->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('distributor_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.distributors.edit', $distributor->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('distributor_delete')
                                                <form action="{{ route('frontend.distributors.destroy', $distributor->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('distributor_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.distributors.massDestroy') }}",
