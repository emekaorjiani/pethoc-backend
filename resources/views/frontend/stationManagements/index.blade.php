        ...
        <div class="col-md-12">
            @can('station_management_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.station-managements.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.stationManagement.title_singular') }}
                        </a>
                        <button class="btn btn-warning" data-toggle="modal" data-target="#csvImportModal">
                            {{ trans('global.app_csvImport') }}
                        </button>
                        @include('csvImport.modal', ['model' => 'StationManagement', 'route' => 'admin.station-managements.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                                    ...
                                    <tr data-entry-id="{{ $stationManagement->id }}">
                                        ...
                                        <td>
                                            @can('station_management_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.station-managements.show', $stationManagement->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('station_management_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.station-managements.edit', $stationManagement->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('station_management_delete')
                                                <form action="{{ route('frontend.station-managements.destroy', $stationManagement->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('station_management_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.station-managements.massDestroy') }}",
