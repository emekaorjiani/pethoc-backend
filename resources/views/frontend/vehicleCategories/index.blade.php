        ...
        <div class="col-md-12">
            @can('vehicle_category_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.vehicle-categories.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $vehicleCategory->id }}">
                                        ...
                                        <td>
                                            @can('vehicle_category_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.vehicle-categories.show', $vehicleCategory->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('vehicle_category_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.vehicle-categories.edit', $vehicleCategory->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('vehicle_category_delete')
                                                <form action="{{ route('frontend.vehicle-categories.destroy', $vehicleCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('vehicle_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.vehicle-categories.massDestroy') }}",
