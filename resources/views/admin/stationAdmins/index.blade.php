...
@can('station_admin_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.station-admins.create') }}">
                        ...
                        <tr data-entry-id="{{ $stationAdmin->id }}">
                            ...
                            <td>
                                @can('station_admin_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.station-admins.show', $stationAdmin->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('station_admin_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.station-admins.edit', $stationAdmin->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('station_admin_delete')
                                    <form action="{{ route('admin.station-admins.destroy', $stationAdmin->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('station_admin_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.station-admins.massDestroy') }}",
