...
@can('fuel_purchase_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.fuel-purchases.create') }}">
                        ...
                        <tr data-entry-id="{{ $fuelPurchase->id }}">
                            ...
                            <td>
                                @can('fuel_purchase_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.fuel-purchases.show', $fuelPurchase->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('fuel_purchase_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.fuel-purchases.edit', $fuelPurchase->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('fuel_purchase_delete')
                                    <form action="{{ route('admin.fuel-purchases.destroy', $fuelPurchase->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('fuel_purchase_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.fuel-purchases.massDestroy') }}",
