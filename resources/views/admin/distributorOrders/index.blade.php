...
@can('distributor_order_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.distributor-orders.create') }}">
                        ...
                        <tr data-entry-id="{{ $distributorOrder->id }}">
                            ...
                            <td>
                                @can('distributor_order_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.distributor-orders.show', $distributorOrder->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('distributor_order_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.distributor-orders.edit', $distributorOrder->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('distributor_order_delete')
                                    <form action="{{ route('admin.distributor-orders.destroy', $distributorOrder->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('distributor_order_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.distributor-orders.massDestroy') }}",
